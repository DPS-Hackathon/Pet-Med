...
function signUp() {
  ...
  auth.createUserWithEmailAndPassword(email.value, password.value).catch(function (error) {
    ...
    // else if (error.Message == "undefined") {alert("Successfully signed up");email.value = "";password.value = ""; if (if_user_is_vet.checked == true){window.location="vetinfo.html"} else {window.location="vets.html";}}
